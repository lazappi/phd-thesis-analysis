...
bc_plot <- bc_data %>%
    ...
    scale_colour_manual(values = c("black", "#EC008C"),
                        labels = c("Empty", "Cell"),
                        name = "Cell Ranger") +
    ggtitle("Cell Ranger droplet selection") +
    ylab("Total counts") +
    theme_minimal() +
...
comp_plot <- ggplot(plot_data, aes(x = Selected, y = Count, fill = Sample)) +
    geom_col() +
    scale_fill_manual(values = c("#EC008C", "#00ADEF", "#8DC63F"),
                      name = "Organoid") +
    ggtitle("Comparison of droplet selection methods") +
    ylab("Number of cells") +
    theme_minimal() +
...
fig <- plot_grid(bc_plot, comp_plot, nrow = 1, labels = "AUTO")
